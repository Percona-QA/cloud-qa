...
eksctl utils associate-iam-oidc-provider --region=eu-west-3 --cluster=${CLUSTER_MAIN} --approve
eksctl create iamserviceaccount --region eu-west-3 --name cloud-map-mcs-controller-manager --namespace cloud-map-mcs-system --cluster ${CLUSTER_MAIN} --attach-policy-arn arn:aws:iam::aws:policy/AWSCloudMapFullAccess --approve --override-existing-serviceaccounts

kubectl create ns mcs && kubens mcs
# configure coredns for each cluster:
...
eksctl utils associate-iam-oidc-provider --region=eu-west-3 --cluster=${CLUSTER_REPL} --approve
eksctl create iamserviceaccount --region eu-west-3 --name cloud-map-mcs-controller-manager --namespace cloud-map-mcs-system --cluster ${CLUSTER_REPL} --attach-policy-arn arn:aws:iam::aws:policy/AWSCloudMapFullAccess --approve --override-existing-serviceaccounts
kubectl create ns mcs && kubens mcs
# configure coredns for each cluster:
kubectl apply -f https://raw.githubusercontent.com/aws/aws-cloud-map-mcs-controller-for-k8s/main/samples/coredns-clusterrole.yaml
kubectl apply -f https://raw.githubusercontent.com/aws/aws-cloud-map-mcs-controller-for-k8s/main/samples/coredns-configmap.yaml
...
########################### Setup VPC peering between clusters vpc ###################################################

#To let traffic cross between main and repl clusters we need to create a VPC peer between our main and replica clusters.
...
#Allow traffic from the EKS VPC and Security group of main cluster to replica one
